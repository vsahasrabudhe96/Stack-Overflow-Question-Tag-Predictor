...
class SOFData(object):
    ...
    def unique_tags(self):
        ...
        self.unq_tag_df["TagId"] = self.unq_tag_df["Tag"]\
                                   .swifter.apply(lambda tag: self.Tag[tag])
        #print()
    ...
    def tag_frequency(self):
        ...
        self.tag_freq["TagId"] = self.tag_freq["Tag"].swifter.apply(lambda tag: self.Tag[tag])
    ...
    def removeTags(self, cname):
        ...
        self.q_df[cname] = self.q_df[cname].parallel_apply(lambda text: bs(text, "lxml").text)
        print()
    ...
    def text_tokenizer(self, cname):
        ...
        self.q_df[dst_cname] = self.q_df[src_cname].swifter.apply(lambda txt: self.text_tokens(txt))
        print()
    ...
    def multi_word_tokenizer(self, cname):
        ...
        self.q_df[cname] = self.q_df[cname].swifter.apply(lambda tokens: self.multi_word_tokens(tokens))
        print()       
    ...
    def filter_stop_words(self, cname):
        ...
        self.q_df[cname] = self.q_df[cname].swifter.apply(lambda words: self.remove_stopWords(words))
        print()       
    ...
    def prepPopularTagsDataFrame(self):
        ...
        self.pop_tags_df["Tag"] = self.q_tags_df["Tag"].swifter.apply(lambda tagList: self.keepPopularTags(tagList))
        self.pop_tags_df["TagId"] = self.pop_tags_df["Tag"].swifter.apply(lambda tagList: self.keepPopularTagIds(tagList))
        self.pop_tags_df["NumTags"] = self.pop_tags_df["Tag"].swifter.apply(lambda tagList: len(tagList))
        print()
...
def main():
    ...
    print(f"Parallel time: {preproc_et} seconds.")
